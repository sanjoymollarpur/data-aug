...
for i in range(612):
    ...
    label_path = "cvc-clinicDB/labels/"+header[1]
    boxes = []
    k=i
    image_list=[]
    ...
    print(boxes)
    # print(header[0])
    image = cv2.imread("cvc-clinicDB/bgr-img/"+header[0])
    ...
    for i in range(2):
       ...
       # print(augmentations["bboxes"][0])
        ...
        with open(f"cvc-aug-labels/{header[0].split('.')[0]}.txt", 'w') as f:
            list = []
            for b1 in box1:
        ...
        filename=f"cvc-aug-img/{header[0].split('.')[0]}.jpg"
        #print(filename)
